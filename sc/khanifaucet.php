<?php
define('host',['Khanifaucet','khanifaucet.site','']);
...
while(true){
    ...
    if(preg_match("/Just a moment.../",$r)){
        print msg(4,"Cloudflare!").n;del();Del_Cok();die;
    }
    if(preg_match("/Daily limit reached/",$r)){
        print msg(4,"Daily limit reached").n;die;
    }
    ...
    if(preg_match("/Daily claim limit/",$r)){
        print msg(4,"Daily claim limit").n;
        die;
    }
    ...
    $limit = Ambil($r,'<h4 class="lh-1 mb-1">','/',4);
    $csrf = Ambil($r,'name="csrf_token_name" id="token" value="','">',1);
    $tok = Ambil($r,'name="token" value="','">',1);
    $ca = Ambil($r,'name="captcha"><option value="','">',1);
    $atb = anti_bot($r);
    $cap = Captcha($r,web."/faucet");
    $data = dataX($csrf ,$tok,$ca,$atb,$cap);
    $post = post(web."/faucet/verify",$data);
    if($post){
        if(preg_match("/title: 'Good job!'/",$post)){
            $r = balance(); 
            $hasil = Ambil($post,"text: '","has been added to your balance",1);
            print msg(2,"Reward ").h."[".p.$hasil.h."]".o."│".p."Balance ".h."[".p.$r["b"].h."]".n;
            print msg(3,"Energy ").h."[".p.$r["e"].h."]".o."│".p."Left ".h."[".p.$limit.h."]".o."│".p."Apikey ".h."[".p.Api_Bal().h."]".n;
            
            print lineX();
        }else{
            print msg(4,"Invalid Claim");sleep(2);print rr;
        }
    }
}
