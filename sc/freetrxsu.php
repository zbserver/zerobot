...
Function h(){
    $h[] = "Host: ".host[1];
    $h[] = "Referer: ".web."/?r=TALrypwEaPW5JhDwg6b54KY3PFJrZkagaa";
...
cl();
ban();
echo msg(1,"Apikey").w3." : ".h.Api_Bal().n;
echo " ".line();
while(true){
    $r =  get(web."/?r=TALrypwEaPW5JhDwg6b54KY3PFJrZkagaa");
    $tok = Ambil($r,'name="session-token" value="','">',1);
    if(preg_match('/data-sitekey="/',$r)){
        $sitekey= Ambil($r,'data-sitekey="','"',1);
    }elseif(preg_match("/data-sitekey='/",$r)){
        $sitekey= Ambil($r,"data-sitekey='","'",1);
    }else{
        echo Error("sitekey Error");sleep(2);goto time;
    }
    $cap = captcha($r,web);
    if(!$cap){continue;}
